...
function concatenateStrings(value1, value2) {
  return value1.concat(value2).toString();
}
...
function isPalindrome(str) {
  const lowStr = str.replace(/[^a-zA-Z]+/g, '').toLowerCase();
  return lowStr === lowStr.split('').reverse().join('').toLowerCase();
}
...
function encodeToRot13(str) {
  return str
    ...
    .map((el) => {
      ...
      if (el === ' ') return ' ';
      if ((code >= 65 && code < 78) || (code >= 97 && code <= 109)) {
        return String.fromCharCode(el.charCodeAt(0) + 13);
      }
      if ((code >= 78 && code <= 90) || (code > 109 && code <= 122)) {
        return String.fromCharCode(el.charCodeAt(0) - 13);
      }
      return el;
    })
...
function getCardId(value) {
  const firstCount = value
    ...
    .join('');
  const secondCount = value.split('').pop();
